...
function LowStockReportPage() {
  ...
  const handleExportPDF = () => exportToPDF('report-container', 'low-stock-report.pdf');
  const handleExportExcel = () => {
    ...
    exportToExcel(excelData, 'low-stock-report.xlsx', 'Low Stock');
  };
  ...
  const filteredAndSortedData = useMemo(() => {
    let filtered = data.filter(item => (item.stock || 0) <= stockThreshold);
    ...
    return filtered;
  }, [data, search, sortBy, sortOrder, stockThreshold]);

  const summaryCards = useMemo(() => {
    const lowStockCount = filteredAndSortedData.length;
    const outOfStockCount = filteredAndSortedData.filter(item => (item.stock || 0) === 0).length;
    const backorderCount = filteredAndSortedData.filter(item => (item.stock || 0) < 0).length;
    const totalLowStockValue = filteredAndSortedData.reduce((sum, item) => sum + (Math.max(item.stock || 0, 0) * (item.cost || 0)), 0);

    return [
      { label: 'Low Stock Items', value: lowStockCount.toString() },
      { label: 'Out of Stock', value: outOfStockCount.toString() },
      { label: 'Backorder Items', value: backorderCount.toString() },
      { label: 'Low Stock Value', value: `Rs. ${totalLowStockValue.toLocaleString(undefined, { minimumFractionDigits: 2 })}` },
    ];
      ...
      <div className="bg-white dark:bg-emerald-950/25 border border-slate-200 dark:border-emerald-400/20 rounded-xl p-5 shadow-sm">
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex items-center gap-2">
            <label className="text-sm font-medium text-slate-700 dark:text-emerald-50">Low Stock Threshold:</label>
            <input
              type="number"
              value={stockThreshold}
              onChange={(e) => setStockThreshold(Number(e.target.value))}
              min="0"
              className="rounded-lg border border-slate-300 dark:border-emerald-400/20 bg-white dark:bg-slate-900 px-3 py-2 text-sm text-slate-700 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/20 w-24"
            />
          </div>
