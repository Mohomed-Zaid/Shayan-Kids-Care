...
function ExportDocument({ mode, range, filters, user, vendor, columns, rows, cards, statement }) {
  return <div id="vendor-report-export" className="vendor-export-document fixed left-[-99999px] top-0 w-[1100px] bg-white p-6 text-black print:static print:w-auto print:p-0">
    ...
    <div className="vendor-print-footer mt-3 border-t border-slate-400 pt-1 text-center text-[8px]">Tiny Bloom · {reportName(mode)} · Page <span className="vendor-page-number"/></div>
  </div>
}
...
export default function VendorReportsPage({ initialMode = 'overview' }) {
  ...
  return <div className="space-y-5">
    <div className="vendor-screen-only space-y-5">
      ...
      <Summary cards={cards}/>
      {['statement','ledger'].includes(mode) && vendor ? <div className="rounded-lg border border-slate-200 bg-white p-4 text-sm dark:border-emerald-800 dark:bg-emerald-950/20"><b>Tiny Bloom · {reportName(mode)}</b><div className="mt-2 grid gap-1 md:grid-cols-2"><p><b>Vendor:</b> {vendor.code || '-'} · {vendor.name}</p><p><b>Period:</b> {range.from} to {range.to}</p><p><b>Address:</b> {vendor.address || '-'}</p><p><b>Phone:</b> {vendor.phone || '-'}</p><p><b>Opening Balance:</b> {money(statement?.opening)}</p><p><b>Closing Balance:</b> {money(statement?.totals.closing)}</p></div></div> : null}
