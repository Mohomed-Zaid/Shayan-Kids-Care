...
function SalesByRepPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const aggregatedData = invoices.reduce((acc, invoice) => {
        const repId = invoice.employees?.id;
        if (!repId) return acc;

        if (!acc[repId]) {
          acc[repId] = {
            repId,
            name: invoice.employees?.name || 'Unknown',
            totalRevenue: 0,
            invoiceCount: 0,
        ...
        acc[repId].totalRevenue += invoice.total_amount || 0;
        acc[repId].invoiceCount += 1;
        if (invoice.customers?.name) {
          acc[repId].customerCount.add(invoice.customers.name);
        }
      ...
      const finalData = Object.values(aggregatedData).map(item => ({
        ...
        avgInvoiceValue: item.invoiceCount > 0 ? item.totalRevenue / item.invoiceCount : 0,
      }));
  ...
  const handleExportPDF = () => {
    exportToPDF('report-container', 'sales-by-rep-report.pdf');
  };
  ...
  const handleExportExcel = () => {
    ...
    exportToExcel(excelData, 'sales-by-rep-report.xlsx', 'Sales by Sales Rep');
  };
  ...
  const summaryCards = useMemo(() => {
    const totalReps = filteredAndSortedData.length;
    const totalRevenue = filteredAndSortedData.reduce((sum, item) => sum + (item.totalRevenue || 0), 0);
    const totalInvoices = filteredAndSortedData.reduce((sum, item) => sum + (item.invoiceCount || 0), 0);
    const totalCustomers = filteredAndSortedData.reduce((sum, item) => sum + (item.customerCount || 0), 0);
    const topRep = filteredAndSortedData[0]?.name || 'N/A';

    return [
      { label: 'Total Sales Reps', value: totalReps.toLocaleString() },
      ...
      { label: 'Customers Served', value: totalCustomers.toLocaleString() },
      { label: 'Top Sales Rep', value: topRep.length > 20 ? topRep.substring(0, 17) + '...' : topRep },
    ];
  ...
  const grandTotalInvoices = useMemo(() => filteredAndSortedData.reduce((sum, item) => sum + (item.invoiceCount || 0), 0), [filteredAndSortedData]);
    ...
    <div id="report-container" className="space-y-6">
      ...
      <div className="bg-white dark:bg-emerald-950/25 border border-slate-200 dark:border-emerald-400/20 rounded-xl overflow-hidden shadow-sm">
        <div className="p-5 border-b border-slate-200 dark:border-emerald-400/20 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Sales Rep Performance</h2>
          <div className="flex items-center gap-2">
            <select
              value={sortBy}
            ...
            >
              <option value="name">Sales Rep Name</option>
        ...
        {paginatedData.length === 0 ? (
          <EmptyState message="No sales rep data for this period" />
        ) : (
          <>
            <table className="w-full text-sm">
              <thead className="bg-slate-50 dark:bg-emerald-900/30 text-slate-500 dark:text-emerald-100/70 border-b border-slate-200 dark:border-emerald-400/20">
                <tr>
                  ...
                  <th className="text-left px-5 py-3 font-semibold text-xs uppercase tracking-wider">Customers</th>
                  <th className="text-left px-5 py-3 font-semibold text-xs uppercase tracking-wider">Avg Invoice</th>
                </tr>
              </thead>
              <tbody>
                {paginatedData.map((item) => (
                  <tr key={item.repId} className="border-b border-slate-100 dark:border-emerald-400/10 hover:bg-slate-50 dark:hover:bg-emerald-500/5 transition-colors">
                    ...
                    <td className="px-5 py-3.5 text-slate-600 dark:text-emerald-100/70">
                      Rs. {item.avgInvoiceValue.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            <div className="p-5 border-t border-slate-200 dark:border-emerald-400/20 bg-slate-50 dark:bg-emerald-900/20">
              <div className="flex justify-end gap-8">
                <div className="text-right">
                  <p className="text-sm text-slate-500 dark:text-emerald-100/60">Total Invoices</p>
                  <p className="text-lg font-bold text-slate-900 dark:text-white">{grandTotalInvoices.toLocaleString()}</p>
                </div>
                <div className="text-right">
                  <p className="text-sm text-slate-500 dark:text-emerald-100/60">Grand Total Revenue</p>
