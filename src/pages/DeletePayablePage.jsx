...
import { Eye, Landmark, Search, Trash2 } from 'lucide-react'

const fmt = (val) => `Rs. ${Number(val ?? 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}`

export default function DeletePayablePage() {
  ...
  const loadRows = async () => {
    ...
    try {
      ...
      const query = supabase
        ...
        .select('id, purchase_id, amount, paid_at, method, bank_name, reference, note, created_at, purchases(ref_no, vendor_id, vendors(name))')
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((r) => {
      const vendor = r.purchases?.vendors?.name ?? ''
      const refNo = r.purchases?.ref_no ?? ''
  ...
  const deletePayment = async () => {
    ...
    try {
      ...
      const { error: delErr } = await supabase.from('purchase_payments').delete().eq('id', deletingPayment.id)
      if (delErr) {
      ...
      logAction({ action: 'delete_payable_payment', targetType: 'purchase_payment', targetId: deletingPayment.id })
      setSelected(null)
      await loadRows()
    } catch (e) {
      console.error(e)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-white border border-slate-200/60 rounded-2xl overflow-hidden shadow-sm dark:bg-emerald-950/25 dark:border-emerald-400/15">
          <div className="p-5 border-b border-slate-100 dark:border-emerald-900/40">
            <div className="text-base font-bold text-slate-900 dark:text-emerald-50">Payables</div>
              ...
              <table className="w-full text-sm">
                <thead className="bg-slate-50 dark:bg-emerald-950/35 text-slate-600 dark:text-emerald-100/80">
                  <tr>
                    ...
                    <th className="text-left px-4 py-3 text-xs uppercase tracking-wider">Vendor</th>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-emerald-900/30">
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan={5} className="px-4 py-10 text-center text-slate-400 dark:text-emerald-100/60">No payables found</td>
                    </tr>
                  ) : (
                    filteredRows.map((r) => (
                      <tr
                      ...
                      >
                        ...
                        <td className="px-4 py-3.5 text-slate-700 dark:text-emerald-50">{r.purchases?.vendors?.name ?? '-'}</td>
        ...
        <div className="bg-white border border-slate-200/60 rounded-2xl overflow-hidden shadow-sm dark:bg-emerald-950/25 dark:border-emerald-400/15">
          <div className="p-5 flex items-center justify-between border-b border-slate-100 dark:border-emerald-900/40">
            <div>
              <div className="text-base font-bold text-slate-900 dark:text-emerald-50">Discard Payable</div>
          ...
          <div className="p-5">
            {!selected ? (
              <div className="py-10 text-center text-slate-400 dark:text-emerald-100/60">
                ...
                <div className="mt-2 text-sm">Select a payable from the left</div>
              </div>
            ) : (
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div className="text-slate-500 dark:text-emerald-100/60">Vendor</div>
                  <div className="text-slate-900 dark:text-emerald-50 font-semibold text-right">{selected.purchases?.vendors?.name ?? '-'}</div>
                ...
                <div className="rounded-xl border border-slate-200 dark:border-emerald-900/40 overflow-hidden">
                  <table className="w-full text-sm">
                    <thead className="bg-slate-50 dark:bg-emerald-950/35 text-slate-600 dark:text-emerald-100/80">
                      <tr>
                        <th className="text-left px-4 py-3 text-xs uppercase tracking-wider">Purchase Ref</th>
                        <th className="text-right px-4 py-3 text-xs uppercase tracking-wider">Paid</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100 dark:divide-emerald-900/30">
                      <tr>
                        <td className="px-4 py-3.5 font-semibold text-slate-900 dark:text-emerald-50">{selected.purchases?.ref_no ?? '-'}</td>
